...
func testCustomConfApp(t *testing.T, context spec.G, it spec.S) {
	var (
	...
	context("when pushing an app with a custom nginx.conf location", func() {
		it.Before(func() {
			var err error
			source, err = occam.Source(filepath.Join("testdata", "custom_conf_app"))
			Expect(err).NotTo(HaveOccurred())
		})
